...
## 15. 17:24 用户批评 v6-lite.2 → 状态说明缺失 + UI 元素不统一

> 用户原话：「**后面跟着的状态说明呢，是否激活，激活了哪个？用户不清楚**，上面的图片只是简单说明，并不是要你做到一模一样啊。**而且 ui 元素要统一！**」

**我上轮的错（根因）**：
1. 把截图当「像素级照搬」——用户给的 `live | jarvis | kws` 红字只是**示意「放 header 一排」这个概念**，不是要裸文字。
2. 自创了 `.qt` 纯文字样式（无边框/无圆角/无状态），**与 header 既有 `.health-pill`、main 既有 `.chip`（MIC/ASR/LLM…）视觉语言割裂** → 违反「UI 元素统一」。
3. **丢了状态说明**：裸文字看不出「哪个激活、是否激活」，用户明确不满意。

**v6-lite.3 修正（落地预览）**：
- **复用 `.chip` 元件**（与 main telemetry 同款：圆角药丸 + 6px 状态点 + 文字），三个模式改成 `<span class="chip mode-chip">`。
- **加激活态 dot**：`.mode-chip .cdot` 默认灰（`var(--text-3)` = 未激活）；`.mode-chip.active` 绿点 + 品牌色边框 + 浅底高亮 = 激活。当前按健康菜单实际态标 `jarvis` 激活（监听中）、`live`/`kws` 未激活。
- **title 悬浮说明**：每个 chip 带 `title`（如「Jarvis 常驻（监听中）」），补「状态说明」缺口。
- **可点切换**：`toggleMode(el)` 切 `.active`（预览本地高亮，真实生效靠后端模式路由）。
- 删除 `.qt` CSS（被 `.mode-chip` 取代，无残留）。

**统一规则（给 UI 组 + 自己，硬约束）**：
- header 的状态指示、main 的 telemetry、健康菜单的 svc —— **状态点语言一致**（绿=正常/激活、灰=未激活、黄=警告）；新增状态类元件**必须复用 `.chip` 或 `.svc`**，禁止自创裸文字/新形态。

...
> 并确认崩溃潮与系统崩溃是同一时刻。**
>
> ## ⚠️ 但本报告的**因果链**部分（§三）已被推翻（同日更正）
>
> **本文曾推断**「DTS APO → AUDIODG 崩溃 → 内核态故障 → `0x20001`」。
>
> **该推断错误。** 决定性反证：**14:00 发生过 60 次签名完全相同的 `dtstech64.dll` 崩溃，
> 当时并无任何 bugcheck**（System 日志自 09-12 完整保留，全天 bugcheck 仅 1 条）。
> 且本机**不存在任何 DTS 内核驱动**（`System32\drivers` 下零个 `*dts*.sys`），
> `dtstech64.dll` 由**纯用户态** `audiodg.exe` 加载，其 `0xc0000409` 是用户态栈溢出。
>
> **⇒ 正确表述：音频崩溃与蓝屏是「时间邻接」，不是因果。**
> 蓝屏原因**未定**，线索更指向虚拟化栈。
> 详见 `bluescreen-rootcause-2026-09-20.md` **§0**（含三条复验过的反驳证据）。
>
> **§一（时间线修正）仍然成立** —— 崩溃潮与系统崩溃确在同一分钟内；
> 但那只是**邻接**证据，不能推出因果。

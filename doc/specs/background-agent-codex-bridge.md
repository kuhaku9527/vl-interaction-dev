...
> 实现: `services/background-agent/agent_provider.py`：`AgentProvider` ABC + `create_agent_provider`；默认 provider = **codex**
> 验证: `tests/test_agent_provider_factory.py` 6 项 + `test_codex_api_recall.py` 通过。

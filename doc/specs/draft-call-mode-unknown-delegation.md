# Spec（草稿）— call 模式"不知道就委派"（N2）

> 状态：**草稿** — 待用户验收后转正式（按 `决策/README.md` §0 治理）。
> 日期：2026-08-13
> 关联：`doc/main/00-main-direction.md` §4.0b N2、`doc/specs/draft-background-agent-codex-bridge.md`（N1，Codex 桥接）、`决策/服务-background-agent.md`（D-049）
> 背景事实：call 模式（`/api/llm/message` 文本直达，`interaction_mode="call"`）使用 `NO_DECISION_SYSTEM_PROMPT`（prompt_constants.py:157），**明确禁止** `</delegation>` decision token——因此模型在 call 模式答"不知道今天几号/天气"时**永远不会自行委派**。这是设计使然，但暴露了体验缺口：用户问了实时/外部信息，模型只能干答"不知道"。

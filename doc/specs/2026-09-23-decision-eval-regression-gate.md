...
> ★ **现状（已由 #169 闭环，2026-09-24）** —— 本节上面两行是 **2026-09-23 的如实历史**，
> **原文保留**（不要改写历史）。**闭环方式**：给**执行器**加一个逐 check 可选字段
> `content_digest`（整份内容的 canonical sha256；缺省 `null` = 不校验，既有 11 条零变更），
> 与既有 `present_when_missing` / `parse_as` **三层并列**。**实测（2026-09-24 复跑）**：
> 镜像里伪造某 variant 的 `criteria_registry` ⇒ **改前 rc=0 → 改后 rc=1**
> （判红该轴两条 check）；健康态与布局重写（indent / sort_keys / CRLF）仍 **rc=0**。
> **未新增 job、未加 `needs`、未新增执行器、未新增退出码** —— option (2) 的目标
> 靠"门禁自己持有摘要"达成（门禁本就跑在 `drift-gate` job 里）。
> ★ **本条不是"给 registry 补 pattern"**：走的是**类级一般化**（§3.3.1 那条结论——
> 正则永远绑不完）。详见 `doc/specs/2026-09-24-decision-eval-content-digest.md`（§1/§3/§4）。
> ★ **新的边界（比"已修"更重要）**：摘要绑**内容**、**不是签名** —— 一份**连声明摘要
> 一起改掉**的伪造产物**仍判绿**，那由**代码评审与 `git diff`** 拦（父 spec §6.1 的同款
> 推理在此适用）。**不得**读成"门禁现在能抵抗蓄意伪造"。
> ★ 另有一条**先于 #169 存在**的边界：把产物压成**无冒号空格**的紧凑 JSON
> （值不变、摘要也不变）会让**正则层**判红（`re.escape` 后的字面空格失配）——
> 与摘要层无关，**已如实登记、未修**（#169 spec §6.2）。
...
7. **W2：`criteria_registry` 伪造** —— **门禁层不拦**，唯一守卫是 canonical 摘要层
   ...
   **未做**，本票不声称已修。
   > ★ **现状（已由 #169 闭环，2026-09-24）**：本条**已不再是遗留项**。
   > 门禁新增逐 check 可选 `content_digest`（整份内容的 canonical sha256），实测
   > **改前 rc=0 → 改后 rc=1**（§6.5 W2 的加注里有完整读数与边界）。
   > **未新增 job / `needs` / 执行器 / 退出码**；既有 11 条 check 零变更。
   > ★ 新的边界同样重要：摘要绑**内容**、不是签名 —— 连摘要一起改掉的伪造仍判绿
   > （属评审与 `git diff` 职责）。详见 `doc/specs/2026-09-24-decision-eval-content-digest.md`。

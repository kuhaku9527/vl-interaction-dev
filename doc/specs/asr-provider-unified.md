...
> 生命周期: **正式**（2026-09-14 转正；原为"草稿 v1"2026-08-13）——用户决策"现在做"：jarvis/live 对话 ASR 也支持云端（提识别率，云端延迟实测不高、兼容 OpenAI 协议可换提供方）
> 实现: 2026-08-13；`services/asr/jarvis/asr_provider.py`（`ASRProvider` ABC + `create_asr_provider` + `JARVIS_ASR_PROVIDER`）
> 验证: `test_asr_provider.py` / `test_qa_asr_provider_boundary.py` 存在；`run-windows.env` 含 `JARVIS_ASR_PROVIDER=cloud`

...
> - **§7 历史环境专属机制**（WorkBuddy 特有，已移出正文、仅存档）
>
...
> 具体收口机制因环境而异：WorkBuddy 用 `.workbuddy/env/cache.env.ps1` + 用户级 `setx`（**历史机制，见 §7**）。
> **本环境（DSH）**：缓存已落在 `.cache/`（实测有 `deepsec/` 等），无需额外脚本。

### 1.1 会话内收口（脚本用）

> **WorkBuddy 历史机制**（已移出正文生效范围，见 §7）：dot-source `.workbuddy/env/cache.env.ps1`。
...
2. **草稿写工作区内**：一次性产物放工作区内的忽略目录（`.cache/`），
   ...
   > 原写"放 `.workbuddy/tmp/`"——那是 WorkBuddy 的环境约定，**已移出**（见 §7）。

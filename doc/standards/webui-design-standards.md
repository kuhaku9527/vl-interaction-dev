...
2. 凡「把路径/值当作唯一导入或定位通道」的测试，**必须加存在性守卫**
   （本项目用的是 `assert <path>.is_dir()`，并让消息直接指出该查哪个下标），
   让路径错误**说人话**。否则它表现为**一批同源错误**（本例 10 条一模一样的
   `ModuleNotFoundError`），**读起来像「被测契约坏了」**——本项目已因此误判过一次。
   > ⚠️ **措辞准确性**：「fail-closed」是**行为描述**，不是对本实现的断言 ——
   > 裸 `assert` 在 **`python -O` 下会被剥离**，那时守卫**不生效**（fail-open）。
   > 本项目测试一律以不带 `-O` 运行（CI 与本地 `pytest` 均如此），故当前有效；
   > 但若要**无条件** fail-closed，应改用 `if not path.is_dir(): raise RuntimeError(...)`。
   > 二者选哪个见 [#151](https://github.com/kuhaku9527/vl-interaction-dev/issues/151) 的后续讨论；
   > **不要因为写了 assert 就宣称「不可绕过」**。
